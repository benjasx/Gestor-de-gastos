...
export const MyReport = ({ txByDate, mes, año }) => {
    ...
    const totalGastos = txByDate.reduce((acc, transaction) => {
        if (transaction.type === 'gasto') { // Asumo que tienes una lógica similar para gastos
            // Convierte transaction.amount a un número flotante antes de sumar
            return acc + parseFloat(transaction.amount);
        }
        return acc;
    ...
    const handleGeneratePdf = () => {
        ...
        const docDefinition = generateTransactionsPdf(txByDate, totalIngresos, totalGastos, saldo, mesConverter(), año);
